...
void WebAssemblyAsmPrinter::EmitEndOfAsmFile(Module &M) {
  ...
  bool has_abi    = false;
  bool has_arisen_action = false;
  bool has_arisen_notify = false;
  for (const auto &F : M) {
     if (F.hasFnAttribute("arisen_wasm_import"))
        has_import = true;
     if (F.hasFnAttribute("arisen_wasm_abi"))
        has_abi = true;
     if (F.hasFnAttribute("arisen_wasm_action"))
        has_arisen_action = true;
     if (F.hasFnAttribute("arisen_wasm_notify"))
        has_arisen_notify = true;
  }
  ...
  if (has_abi) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".arisen_abi";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("arisen_wasm_abi")) {
           StringRef abi = F.getFnAttribute("arisen_wasm_abi").getValueAsString();
  ...
  if (has_arisen_action) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".arisen_actions";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("arisen_wasm_action")) {
           StringRef action_name = F.getFnAttribute("arisen_wasm_action").getValueAsString();
  ...
  if (has_arisen_notify) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".arisen_notify";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("arisen_wasm_notify")) {
           StringRef name = F.getFnAttribute("arisen_wasm_notify").getValueAsString();
