...
#define DEBUG_TYPE "arisen_apply"
...
namespace {
  // ArisenApply - Mutate the apply function as needed 
  struct ArisenApplyPass : public FunctionPass {
    static char ID; 
    ArisenApplyPass() : FunctionPass(ID) {}
    bool runOnFunction(Function &F) override {
       if (F.hasFnAttribute("arisen_wasm_entry") || F.getName().equals("apply")) {
...
char ArisenApplyPass::ID = 0;
static RegisterPass<ArisenApplyPass> X("apply_fixup", "Arisen Apply Fixups");

static void registerArisenApplyPass(const PassManagerBuilder&, legacy::PassManagerBase& PM) { PM.add(new ArisenApplyPass()); }
static RegisterStandardPasses RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible, registerArisenApplyPass);
