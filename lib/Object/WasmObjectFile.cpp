...
Error WasmObjectFile::parseArisenABISection(ReadContext& Ctx) {
   StringRef sr = readString(Ctx);
   arisen_abi = sr;

   if (Ctx.Ptr != Ctx.End)
      return make_error<GenericBinaryError>("arisen abi section ended prematurely",
                                          object_error::parse_failed);
   return Error::success();
...
Error WasmObjectFile::parseCustomSection(WasmSection &Sec, ReadContext &Ctx) {
  if (Sec.Name == ".imports") {
     if (Error Err = parseAllowedSection(Ctx))
        return Err;
  } else if (Sec.Name == ".arisen_abi") {
     if (Error Err = parseArisenABISection(Ctx))
        return Err;
  } else if (Sec.Name == ".arisen_actions") {
     if (Error Err = parseActionsSection(Ctx))
        return Err;
  } else if (Sec.Name == ".arisen_notify") {
